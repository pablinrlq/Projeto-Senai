...
import { CreateAtestadoSchema, UpdateAtestadoStatusSchema, User } from '@/lib/validations/schemas';
import { ZodError } from 'zod';
import { Timestamp } from 'firebase-admin/firestore';
import { verifyAuth } from '@/lib/authMiddleware';
import { uploadImageToStorage, uploadImageToPublicFolder } from '@/lib/firebase/storage';
...
export const GET = withFirebaseAdmin(async (req, db) => {
  ...
  if (currUser.get('cargo') === 'USUARIO' && (!userId || userId !== currUser.id)) {
  ...
  const { data, error } = await safeFirestoreOperation(async () => {
    ...
    if (userId) {
      atestadosQuery = atestadosQuery.where('id_usuario', '==', userId);
    }
    if (status) {
    ...
    const paginatedQuery = atestadosQuery.orderBy('createdAt', 'desc').offset(offset).limit(limit);
    const atestadosSnapshot = await paginatedQuery.get();

    // Buscar usuários relacionados
    const userIds = Array.from(new Set(atestadosSnapshot.docs.map(doc => doc.data().id_usuario)));
    const userDocs = await db.getAll(...userIds.map(id => db.collection('usuarios').doc(id)));
    const usuarios = new Map<string, Pick<User, 'id' | 'nome' | 'email' | 'ra'>>();
    ...
    const atestados = atestadosSnapshot.docs.map(doc => {
      ...
      return {
        ...
        data_fim: data.data_fim?.toDate() ?? null,
    ...
    if (imageFile && imageFile instanceof File) {
      try {
        if (process.env.HAS_STORAGE_BUCKET === 'true') {
          const uploadResult = await uploadImageToStorage(imageFile, authResult.uid, 'atestados');
          imageUrl = uploadResult.url;
          imagePath = uploadResult.path;
        } else {
          const uploadResult = await uploadImageToPublicFolder(imageFile, authResult.uid, 'atestados');
          imageUrl = uploadResult.url;
          imagePath = uploadResult.path;
        }
      } catch (uploadError) {
        console.error('Erro ao fazer upload da imagem:', uploadError);
        return NextResponse.json({ error: 'Falha ao enviar imagem do atestado' }, { status: 500 });
      }
    }

    const { data: createdData, error } = await safeFirestoreOperation(async () => {
...
export const PATCH = withFirebaseAdmin(async (req, db) => {
  try {
    ...
    if (!atestadoId) {
      return NextResponse.json({ error: 'ID do atestado é obrigatório' }, { status: 400 });
    }
