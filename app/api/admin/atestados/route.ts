...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get all atestados for admin review
    ...
    const atestados = await Promise.all(
      atestadosSnapshot.docs.map(async (doc) => {
        ...
        // Get user info
        let usuario = null;
        if (atestadoData.userId) {
          const userDoc = await db.collection('usuarios').doc(atestadoData.userId).get();
          if (userDoc.exists) {
        ...
        return {
          id: doc.id,
          data_inicio: atestadoData.data_inicio,
          data_fim: atestadoData.data_fim,
          motivo: atestadoData.motivo || '',
          status: atestadoData.status || 'pendente',
          imagem: atestadoData.imagem_url || '',
          createdAt: atestadoData.createdAt?.toDate()?.toISOString() || new Date().toISOString(),
          observacoes_admin: atestadoData.observacoes_admin || '',
          usuario,
        };
      })
    );
