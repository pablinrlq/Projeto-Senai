...
export async function PATCH(
  ...
  try {
    ...
    if (status === 'rejeitado' && (!observacoes_admin || !observacoes_admin.trim())) {
      return NextResponse.json(
    ...
    // Update atestado with admin review
    const updateData: {
      status: string;
      reviewedAt: Date;
      reviewedBy: string;
    ...
    if (observacoes_admin) {
      updateData.observacoes_admin = observacoes_admin.trim();
    }

    await db.collection('atestados').doc(atestadoId).update(updateData);
