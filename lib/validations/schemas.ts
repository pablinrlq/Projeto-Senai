...
export const UserSchema = z.object({
  ...
  cargo: z.enum(['ADMINISTRADOR', 'USUARIO', 'FUNCIONARIO'], {
    errorMap: () => ({ message: 'Cargo deve ser ADMINISTRADOR, USUARIO ou FUNCIONARIO' })
  }),
...
export const AtestadoSchema = z.object({
  ...
  status: z.enum(['pendente', 'aprovado', 'rejeitado'], {
    errorMap: () => ({ message: 'Status deve ser pendente, aprovado ou rejeitado' })
  }).default('pendente'),
  createdAt: z.string().optional(),
  updatedAt: z.string().optional(),
});
...
export const CreateAtestadoSchema = z.object({
  ...
  imagem_atestado: z.instanceof(File).refine(file => file.size <= MAX_FILE_SIZE, {
...
export const BrazilianDateSchema = z.string().refine(
  ...
  {
    message: 'Data deve estar no formato "3 de novembro de 2025 às 00:00:00 UTC-3"'
  }
);

// Validation for date range
export const AtestadoDateRangeSchema = z.object({
...
export type CreateAtestadoWithUserData = z.infer<typeof CreateAtestadoWithUserSchema>;
